...
let from_file path =
  ...
  (*On regarde le lien du prof, il suffit d'afficher le début du texte et 
     ensuite de gérer les différents arcs. Utiliser la fonction iter*)
  let export f gr = 
    let file = open_out f in
    let func id1 id2 a = fprintf file "      %d -> %d [label = \"%s\"];\n"id1 id2 a in
